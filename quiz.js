function initQuiz() {
    ...
    window.addEventListener('beforeunload', function (e) {
        if (!testSubmitted) { e.preventDefault(); e.returnValue = ''; }
    });
    ...
    function submitTest() {
        ...
        questions.forEach((q) => {
            ...
            if (q.type === 'single_choice') {
                ...
                if (isCorrect) { totalRawScore += questionPoints; correctFullTasksCount++; }
                ...
                if (block) {
                    ...
                    labels.forEach((label, idx) => {
                        ...
                        if (q.correctAnswer !== null && idx === q.correctAnswer) label.classList.add('correct-answer');
            ...
            if (q.type === 'matching') {
                ...
                if (subItemsCorrect) { totalRawScore += matchingPoints; correctFullTasksCount++; }
                else questionCorrect = false;
            }
            ...
            if (q.type === 'open_ended') {
                ...
                q.subQuestions.forEach((sub) => {
                    ...
                    if (sub.id === 'a' && isSubCorrect) { totalRawScore += pointsA; partACorrect = true; }
                    if (sub.id === 'b' && isSubCorrect) { totalRawScore += pointsB; partBCorrect = true; }
                    ...
                    if (input) {
                        ...
                        if (!isSubCorrect) {
                            const hint = document.createElement('span');
                            hint.className = 'correct-hint';
                            if (correctAnswer !== null && correctAnswer !== undefined) hint.innerHTML = escapeHtmlOutsideMath(`Правильный ответ: ${correctAnswer}`);
        ...
        const currentResult = { score: finalScore, correctCount: correctFullTasksCount, total: totalTasksCount, level: gradeInfo.level };
        ...
        if (existingData) {
            ...
            } catch (error) { console.error('Ошибка localStorage:', error); }
        }
    ...
    function renderResultBox(score, correctCount, totalCount, gradeInfo, details) {
        ...
        details.forEach((item) => {
            ...
            if (!item.answered) { cell.classList.add('result-cell--empty'); cell.title = 'Вопрос без ответа'; }
