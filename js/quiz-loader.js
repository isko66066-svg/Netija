(function () {
    ...
    if (!document.getElementById('test-access-styles')) {
        ...
        style.textContent = `
            .test-title.test-title--access{max-width:720px;margin:56px auto 14px;padding:0 24px;text-align:center;font-size:clamp(26px,4vw,34px);line-height:1.2;font-weight:800;letter-spacing:-.7px;color:#19191B}
    ...
    function getCurrentUser(){
        try{return JSON.parse(localStorage.getItem('netija_user')||'null')}catch(e){return null}
    }
    ...
    async function checkDailyAccess(){
        ...
        try{
            ...
            showAccessMessage('Ошибка доступа','Не удалось проверить доступ к тесту. Попробуйте ещё раз.');return false;
        }catch(error){console.error('Daily test access error:',error);showAccessMessage('Ошибка соединения','Не удалось связаться с сервером. Попробуйте ещё раз.');return false}
    }

    function applyQuestionImages(){
        if(typeof questions==='undefined'||!Array.isArray(questions))return;
        const imageMap={
            '6':{23:'images/2-mart_2-img/23.svg',25:'images/2-mart_2-img/25.svg',26:'images/2-mart_2-img/26.svg',27:'images/2-mart_2-img/27.svg'},
            '7':{23:'images/7-mart_1-img/23.svg',25:'images/7-mart_1-img/25.svg',26:'images/7-mart_1-img/26.svg',27:'images/7-mart_1-img/27.svg'}
        };
        const map=imageMap[String(testId)]||{};
        questions.forEach(q=>{if(map[q.id])q.image=map[q.id]});
    }

    function applyMatchingQuestions(){
        if(typeof questions==='undefined'||!Array.isArray(questions))return;
        const q33=questions.find(q=>q.id===33), q34=questions.find(q=>q.id===34), q35=questions.find(q=>q.id===35);
    ...
    async function loadQuiz(){
        const allowed=await checkDailyAccess(); if(!allowed)return; restoreTestUI();
        const oldScript=document.querySelector('script[data-questions-loader]'); if(oldScript)oldScript.remove();
        const script=document.createElement('script'); script.src=`js/questions/natcert-${testId}.js`; script.dataset.questionsLoader='true';
        script.onload=function(){applyQuestionImages();applyMatchingQuestions();addMatchingVisualFix();if(typeof initQuiz==='function')initQuiz()};
