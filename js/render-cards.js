...
function createSvgIcon(type, className) {
    ...
    const icons = {
        ...
        arrow: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 12H19M13 6L19 12L13 18" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`
    };
...
function renderStatus(statusBox, result) {
    ...
    statusBox.innerHTML = `
        <div class="card-badge ${badgeClass}">
            <span class="card-badge__icon" aria-hidden="true">
                ${createSvgIcon('star', '').outerHTML}
            </span>
            <span>Лучший результат: ${result.score} (${result.correctCount}/${result.total}) — ${result.level}</span>
        </div>
    `;
}
