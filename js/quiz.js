function initQuiz() {
    ...
    // =========================================================
    // СОЗДАНИЕ ТЕКСТА С MATHJAX
    // =========================================================

    // ВАЖНО: экранируем < и > вне зависимости от контекста, чтобы
    // браузер не принимал их за начало HTML-тега (например, в "$a<b<c$"),
    // MathJax корректно распознаёт &lt; и &gt; внутри формул как < и >.
    ...
    function createMathText(className, text) {
        ...
        // ВАЖНО:
        // innerHTML нужен вместо textContent,
        // чтобы MathJax видел $...$, \frac, \sqrt и т.д.
    ...
    // =========================================================
    // ПЕРЕВОД LaTeX В ПРОСТОЙ ТЕКСТ
    // Используется только для <option> внутри <select>,
    // так как этот тег не поддерживает HTML/MathJax вообще —
    // браузер показывает там исключительно чистый текст.
    // =========================================================
    function latexToPlainText(text) {
        ...
        // \sqrtN (без фигурных скобок, например \sqrt3) -> √N
        result = result.replace(/\\sqrt(\d+)/g, '√$1');

        // \operatorname{tg}/\operatorname{ctg} -> tg/ctg
        result = result.replace(/\\operatorname\{([^{}]+)\}/g, '$1');

        // Убираем оставшиеся одиночные фигурные скобки после замен
        result = result.replace(/[{}]/g, '');

        // Убираем экранирование пробелов вида \ 
        result = result.replace(/\\,|\\ /g, ' ');
    ...
    async function renderQuestions() {
        ...
        questions.forEach((q) => {
            ...
            if (q.subtitle) {
                ...
                subtitle.innerHTML =
                    escapeHtmlOutsideMath(q.subtitle);
            ...
            if (q.type === 'single_choice') {
                ...
                q.options.forEach((option, optIndex) => {
                    ...
                    optionText.innerHTML =
                        escapeHtmlOutsideMath(`${letters[optIndex] || ''}) ${option}`);
            ...
            if (q.type === 'matching') {
                ...
                if (q.context) {
                    ...
                    context.innerHTML =
                        escapeHtmlOutsideMath(q.context);
                ...
                q.items.forEach((item) => {
                    ...
                    itemText.innerHTML =
                        escapeHtmlOutsideMath(`${item.id}. ${item.text}`);
                    ...
                    Object.entries(
                        ...
                        /*
                         * ВАЖНО:
                         * <option> не поддерживает HTML/MathJax —
                         * браузер показывает только чистый текст.
                         * Поэтому переводим LaTeX в простой текст
                         * (например, $64\sqrt3$ -> 64√3) вручную,
                         * без участия MathJax.
                         */

                        opt.textContent =
                            `${letter}) ${latexToPlainText(value)}`;
            ...
            if (q.type === 'open_ended') {
                ...
                q.subQuestions.forEach((sub) => {
                    ...
                    subText.innerHTML =
                        escapeHtmlOutsideMath(sub.text || '');
        ...
        // Обрабатываем ВСЕ формулы внутри теста
    ...
    if (submitBtn) {
        ...
        console.error(
            'Quiz: кнопка #submitBtn не найдена'
        );
    }
    ...
    function submitTest() {
        ...
        questions.forEach((q) => {
            ...
            if (q.type === 'matching') {
                ...
                // ВАЖНО:
                // matching получает 2.2 только если
            ...
            if (q.type === 'open_ended') {
                ...
                q.subQuestions.forEach((sub) => {
                    ...
                    /*
                     * null означает, что правильный ответ
                     * не задан.
                     *
                     * Это НЕ меняет систему подсчёта:
                     * такой ответ просто не будет правильным.
                     */
                ...
                /*
                 * НЕ МЕНЯЕМ:
                 * полное задание считается правильным,
                 * только если A и B правильные.
                 */
    ...
    function renderResultBox(
        ...
        // На случай если в результате есть формулы
